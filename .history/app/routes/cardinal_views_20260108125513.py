...
async def generate_cardinal_views_endpoint(
    ...
    num_steps: int = Query(30, ge=20, le=75, description="Quality (20=fast, 50=balanced, 75=best)"),
    db: Session = Depends(get_db),
):
    """
    High-quality generation of 4 cardinal views: top, bottom, left, right
    
    Uses Zero123 AI model for accurate results:
    - AI-powered 3D understanding
    - Realistic view synthesis
    - High quality output
    - ~15-20 seconds per view
    
    Response time: ~60-80 seconds for all 4 views (accurate AI-generated)
    Cached results returned instantly if already generated
    """
    ...
    # 🚀 CHECK CACHE - Return cached views if they exist
    if product.side_views_keys:
        try:
            existing_views = json.loads(product.side_views_keys)
            # Check if we have views for this specific angle
            angle_int = int(angle)
            expected_views = [f"top_{angle_int}", f"bottom_{angle_int}", f"left_{angle_int}", f"right_{angle_int}"]
            
            # If all 4 cardinal views exist for this angle, return cached results
            cached_views = {k: v for k, v in existing_views.items() if k in expected_views}
            if len(cached_views) == 4:
                print(f"✅ Returning cached views for {product.sku_code} at {angle}°")
                return {
                    "success": True,
                    "status": "cached",
                    "angle": angle,
                    ...
                    "views": {
                        ...
                        for name, key in cached_views.items()
                    },
                    "performance": {
                        "inference_steps": 0,
                        "estimated_time": "0 seconds (cached)",
                        "method": "cache",
                        "quality": "cached"
                    }
                }
    ...
    try:
        ...
        # 3️⃣ Generate 4 cardinal views with AI (High Quality)
        print(f"🚀 Generating cardinal views at {angle}° with {num_steps} steps...")
        
        from app.zero123_fast import generate_cardinal_views_fast
        ...
        return {
            ...
            "performance": {
                ...
                "estimated_time": f"{num_steps * 2}-{num_steps * 3} seconds",
                "method": "zero123_ai",
                "quality": "high"
            }
        }
...
async def generate_multi_cardinal_views_endpoint(
    ...
    Uses Zero123 AI model for accurate results
    High quality, realistic view synthesis
    Note: Generates 4 views × number of angles (takes longer)
    """
    ...
    try:
        ...
        from app.zero123_fast import generate_multi_angle_cardinal_views_fast
