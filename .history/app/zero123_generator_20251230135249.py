...
def generate_perspective_views(image):
    """
    Generate simulated perspective views using image transformations.
    This is a lightweight alternative when Zero123 is not available.
    
    Creates top, bottom, left, and right views using PIL transformations.
    ...
    try:
        ...
        # 1. Top View - Apply perspective transformation
        top_view = image.copy()
        # Simulate looking down - slight trapezoid effect
        top_view = top_view.resize((int(width * 0.85), int(height * 0.85)), Image.Resampling.LANCZOS)
        # Add shadow/depth effect
        enhancer = ImageEnhance.Brightness(top_view)
        top_view = enhancer.enhance(1.1)
        # Center it on white background
        result = Image.new('RGB', (width, height), 'white')
        result.paste(top_view, ((width - top_view.width) // 2, (height - top_view.height) // 2))
        views['top'] = result
        
        # 2. Bottom View - Inverse of top
        bottom_view = image.copy()
        bottom_view = bottom_view.resize((int(width * 0.9), int(height * 0.9)), Image.Resampling.LANCZOS)
        enhancer = ImageEnhance.Brightness(bottom_view)
        bottom_view = enhancer.enhance(0.9)
        result = Image.new('RGB', (width, height), 'white')
        result.paste(bottom_view, ((width - bottom_view.width) // 2, (height - bottom_view.height) // 2))
        views['bottom'] = result
        
        # 3. Left View - Perspective from left side
        left_view = image.copy()
        # Apply horizontal squeeze to simulate side angle
        left_view = left_view.resize((int(width * 0.7), height), Image.Resampling.LANCZOS)
        # Add gradient shadow on right side
        result = Image.new('RGB', (width, height), 'white')
        result.paste(left_view, (0, 0))
        views['left'] = result
        
        # 4. Right View - Mirror of left
        right_view = image.copy()
        right_view = right_view.resize((int(width * 0.7), height), Image.Resampling.LANCZOS)
        result = Image.new('RGB', (width, height), 'white')
        result.paste(right_view, (width - right_view.width, 0))
        views['right'] = result
