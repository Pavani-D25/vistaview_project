...
if DEPENDENCIES_AVAILABLE:
    ...
    # NOTE: Instead of using the heavy Stable Diffusion pipeline above,
    # we provide fast depth-based side-view generation helpers here.

    def _estimate_depth_map(image: Image.Image) -> Optional[np.ndarray]:
        """Lightweight depth estimation using edges and intensity (CPU-only, fast)."""
        try:
            ...
            # Edges (for structure awareness)
            edges = cv2.Canny(blurred, 50, 150)

            # Pseudo-depth: inverted intensity
            depth_map = 255 - blurred
            depth_map = depth_map.astype(np.float32) / 255.0

            # Combine with edges for better structure
    ...
    def _generate_view_from_depth(
        image: Image.Image,
        depth_map: np.ndarray,
        direction: str,
        angle: float,
    ) -> Optional[Image.Image]:
        """Generate a left or right view using depth-based 3D-ish warping.

        The angle controls how strong the parallax effect is.
        """
        try:
            ...
            # Base displacement (15% for ~30°), scale with requested angle
            base_displacement = w * 0.15
            angle_scale = max(0.5, min(1.5, angle / 30.0))  # clamp for stability
            max_displacement = base_displacement * angle_scale
            ...
            # Remap with depth-based displacement
            warped = cv2.remap(
                img_cv,
                new_x,
                new_y,
            ...
            # Perspective tweak for more realistic view
            ...
            print(f"      ✅ Generated {direction} view at {angle}°")
            return result

        except Exception as e:
            print(f"      ⚠️  View generation failed for {direction} at {angle}°: {e}")
            return None
    ...
    def generate_side_views(
        ...
        """Fast side views using depth-based warping instead of heavy models."""
        # Respect the flag but keep behavior simple
        if not use_zero123:
            return None, None, "zero123_disabled"

        try:
            depth_map = _estimate_depth_map(image)
            if depth_map is None:
                return None, None, "depth_estimation_failed"

            left_view = _generate_view_from_depth(image, depth_map, "left", angle)
            right_view = _generate_view_from_depth(image, depth_map, "right", angle)
    ...
    def generate_multi_angle_side_views(
        ...
        """Fast multi-angle views using the same depth-based warping.

        Returns keys like "left_15", "right_30", ...
        """
        if not use_zero123:
            return {}, "zero123_disabled"
