...
View generation happens on-demand using production pipeline
"""
import os
import re
import uuid
...
# Try to import production pipeline
try:
    from .production_pipeline import generate_production_views
    PRODUCTION_PIPELINE_AVAILABLE = True
except ImportError:
    PRODUCTION_PIPELINE_AVAILABLE = False
    print("⚠️  Production pipeline not available")

# Fallback to Zero123 if needed
try:
    from .zero123_views import generate_side_views, generate_multi_angle_side_views
    ZERO123_AVAILABLE = True
except ImportError:
    ZERO123_AVAILABLE = False
    # Dummy functions when Zero123 is not available
    def generate_side_views(*args, **kwargs):
        return None, None, 'zero123_not_available'
    def generate_multi_angle_side_views(*args, **kwargs):
        return {}, 'zero123_not_available'
...
def generate_multi_angle_views(image: Image.Image, sku: str, angles: List[float] = [15.0, 30.0, 45.0]) -> Tuple[Dict[str, Image.Image], str]:
    ...
    try:
        ...
        views_dict, status = generate_multi_angle_side_views(
            ...
            angles=angles,
            use_zero123=True,
            num_inference_steps=75,  # Balance between quality and speed
        )
...
def ingest_pdf(pdf_bytes: bytes, db: Session, pdf_filename: str = "catalog.pdf") -> Dict:
    ...
    for page_num in range(pages_processed):
        try:
            ...
            # Do NOT generate side views at ingestion time anymore.
            # Side views will be generated on-demand when the user clicks a product.
            side_views_keys = {}
            left_view_key = None
            right_view_key = None
            view_status = 'not_generated'
