...
def enhance_image(img: Image.Image) -> Optional[Image.Image]:
    """
    Enhance low-resolution images using high-quality upscaling
    
    Args:
        img: PIL Image object
    
    Returns:
        Enhanced PIL Image or None if enhancement not needed
    """
    width, height = img.size
    max_dimension = max(width, height)
    
    # Only enhance if image is below threshold
    if max_dimension >= ENHANCEMENT_THRESHOLD:
        return None
    
    # Calculate scale factor to reach target size
    scale_factor = TARGET_ENHANCEMENT_SIZE / max_dimension
    ...
    enhanced = enhancer.enhance(1.5)  # Increase sharpness by 50%
    
    # Enhance contrast slightly
    contrast_enhancer = ImageEnhance.Contrast(enhanced)
    enhanced = contrast_enhancer.enhance(1.1)  # Increase contrast by 10%
    
    print(f"      🔍 Enhanced image from {width}x{height} to {new_width}x{new_height}")
    return enhanced
