...
Extracts images, metadata, and creates product records with left/right view generation using Zero123
"""
import os
import re
import uuid
...
from .zero123_generator import generate_side_views
...
def ingest_pdf(pdf_bytes: bytes, db: Session, pdf_filename: str = "catalog.pdf") -> Dict:
    ...
    for page_num in range(pages_processed):
        try:
            ...
            left_view, right_view, view_status = generate_left_right_views(main_img, sku)
            ...
            if left_view:
                ...
                left_buffer.seek(0)
                left_view_key = f"images/{session_id}/views/{left_filename}"
            ...
            if right_view:
                ...
                right_buffer.seek(0)
                right_view_key = f"images/{session_id}/views/{right_filename}"
            ...
            if view_status != 'success':
                ...
                not_possible_content = f"View generation not possible: {view_status}".encode('utf-8')
