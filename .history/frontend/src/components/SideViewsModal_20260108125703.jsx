...
const SideViewsModal = ({ product, onClose }) => {
  ...
  const [useFastMode] = useState(true); // Toggle for fast mode
  const [isCached, setIsCached] = useState(false);

  // Start generating views when modal opens
  useEffect(() => {
    // Check if we already have side_views_urls populated
    if (product.side_views_urls && Object.keys(product.side_views_urls).length >= 4) {
      console.log('✅ Using cached views from product data');
      setViews(product.side_views_urls);
      setIsCached(true);
    } else if (useFastMode) {
      generateFastCardinalViews();
    } else {
      generateNextAngle(0);
    }
  ...
  // Fast mode: Generate all 4 cardinal views (top, bottom, left, right) at once
  const generateFastCardinalViews = async () => {
    setLoading(true);
    setError(null);

    try {
      // Generate all views at 30 degrees with 25 inference steps (faster with good quality)
      const response = await fetch(
        `/api/products/${product.id}/generate-cardinal-views?angle=30&num_steps=25`,
        {
          method: 'POST',
        }
      );
      ...
      // Update views with all 4 cardinal directions
      setViews((prev) => ({
        ...prev,
        ...(data.views || {}),
      }));
  ...
  // Legacy mode: Generate views angle-by-angle
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-body">
          {isCached && views && Object.keys(views).length > 0 && (
            <div className="cached-indicator">
              <p>⚡ Loaded from cache (instant)</p>
            </div>
          )}
          ...
          {loading && (
            <div className="loading-state">
              ...
              <p>🤖 Generating AI-powered cardinal views...</p>
              <p className="loading-note">
                {useFastMode 
                  ? 'Creating high-quality top, bottom, left, right views (~60-90 seconds)' 
