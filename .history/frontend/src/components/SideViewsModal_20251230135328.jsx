          ...
          {views && !loading && (
            <div className="views-grid">
              {Object.entries(views).map(([viewName, url]) => {
                // Format view name (e.g., "left" -> "Left View", "top" -> "Top View")
                const label = `${viewName.charAt(0).toUpperCase() + viewName.slice(1)} View`;
                
                // Emoji for each view type
                const emojis = {
                  'top': '⬆️',
                  'bottom': '⬇️',
                  'left': '⬅️',
