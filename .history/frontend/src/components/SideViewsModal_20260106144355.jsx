...
const SideViewsModal = ({ product, onClose }) => {
  ...
  const [useFastMode, setUseFastMode] = useState(true); // Toggle for fast mode
  ...
  const generateFastCardinalViews = async () => {
    ...
    try {
      // Generate all views at 30 degrees with 20 inference steps (fast!)
      const response = await fetch(
        `/api/products/${product.id}/generate-cardinal-views?angle=30&num_steps=20`,
        {
          method: 'POST',
        }
      );
        ...
        <div className="modal-body">
          {loading && (
            <div className="loading-state">
              ...
              <p>🚀 Generating cardinal views (Fast Mode)...</p>
              <p className="loading-note">
                {useFastMode 
                  ? 'Creating top, bottom, left, right views at 30° (~30-40 seconds)' 
